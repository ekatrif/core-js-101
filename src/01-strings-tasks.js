...
function extractNameFromTemplate(string) {
  const array = string.slice(0, string.length - 1).split(' ');
...
function encodeToRot13(str) {
  const result = str.split('').map((char) => {
    if (char.match(/[A-Za-z]/)) {
      const encodedCharCode = char.charCodeAt() + 13;
      if ((char <= 'Z' && encodedCharCode > 90) || (char <= 'z' && encodedCharCode > 122)) {
        return String.fromCharCode(encodedCharCode - 26);
      }
